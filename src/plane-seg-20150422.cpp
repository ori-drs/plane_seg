...
public:
  ...
  Result go() {
    ...
    for (int i = 0; i < height; ++i) {
      for (int j = 0; j < width; ++j) {
        auto& label = labels.at<int32_t>(i,j);
        label = lut[label];
      }
    }
  ...
  Solution estimate(const std::vector<int>& iIndices) const {
    ...
    if (mSinglePoint) {
      ...
      if (iIndices.size() == 3) {
        ...
        MatrixX3f data(iIndices.size(),3);
...
public:
  ...
  Result go() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      Entry entry;
      entry.mEdgeIndex = i;
      entry.mPointMin = minPoint.head<3>();
      entry.mPointMax = maxPoint.head<3>();
      entry.mArea = (entry.mPointMax-entry.mPointMin).head<2>().norm();
      entry.mTransform = transform;
      if (entry.mArea < bestEntry.mArea) {
        bestEntry = entry;
        std::cout << "AREA " << bestEntry.mArea << std::endl;
      }
    }

    Eigen::Vector3f p0(bestEntry.mPointMin[0], bestEntry.mPointMin[1], 0);
    Eigen::Vector3f p1(bestEntry.mPointMin[0], bestEntry.mPointMax[1], 0);
    Eigen::Vector3f p2(bestEntry.mPointMax[0], bestEntry.mPointMax[1], 0);
    Eigen::Vector3f p3(bestEntry.mPointMax[0], bestEntry.mPointMin[1], 0);
    Result result;
    result.mPlane = mPlane;
    result.mPolygon = { p0, p1, p2, p3 };
    for (auto& pt : result.mPolygon) {
      pt = bestEntry.mTransform.inverse()*pt;
    }
...
int main() {
  ...
  */
  ...
  std::vector<int> inlierImage(width*height);
  std::fill(inlierImage.begin(), inlierImage.end(), 0);
  std::vector<int> iterImage(width*height);
  std::fill(iterImage.begin(), iterImage.end(), 0);
  for (int i = 0; i < height; ++i) {
    for (int j = 0; j < width; ++j) {
      ...
      if (pts.size() >= 3) {
        ...
        normal = res.mSolution.mPlane.head<3>();
        curvature = res.mSolution.mCurvature;
        inlierImage[i*width+j] = res.mInliers.size();
        iterImage[i*width+j] = res.mNumIterations;
  ...
  {
    std::ofstream ofs("/home/antone/temp/inliers.txt");
    for (int i = 0; i < height; ++i) {
      for (int j = 0; j < width; ++j) {
        ofs << inlierImage[i*width+j] << " ";
      }
      ofs << std::endl;
    }
    ofs.close();

    ofs.open("/home/antone/temp/iters.txt");
    for (int i = 0; i < height; ++i) {
      for (int j = 0; j < width; ++j) {
        ofs << iterImage[i*width+j] << " ";
      }
      ofs << std::endl;
    }
    ofs.close();
  ...
  ofs.close();
  ofs.open("/home/antone/temp/planes.txt");
  ...
  for (int i = 0; i < result.mLabels.rows; ++i) {
    for (int j = 0; j < result.mLabels.cols; ++j) {
      int label = result.mLabels.at<int32_t>(i,j);
      if (label <= 0) continue;
      cloudMap[label].push_back
        (organizedCloud->points[i*width+j].getVector3fMap());
    }
  }
  struct Plane {
  ...
  for (auto& plane : planes) {
    ...
    for (auto& p : result.mPolygon) p = pose*p;
    for (auto& p : result.mConvexHull) p = pose*p;
    results.push_back(result);
  }

  ofs.open("/home/antone/temp/boxes.txt");
  ...
  ofs.open("/home/antone/temp/hulls.txt");
  for (int i = 0; i < (int)results.size(); ++i) {
    auto& res = results[i];
